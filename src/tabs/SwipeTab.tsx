...
import { useAbortController, usePreloadNextCard } from '@/hooks';
import { vibrate, playSwipe } from '@/utils';
import { Icons, ToggleButtons, EmptyState, Button } from '@/components/ui';
    ...
    <div className="flex flex-col h-[75vh] items-center justify-center relative">
      ...
      {currentCard ? (
        <div className="w-full max-w-sm flex-1 flex flex-col items-center justify-center relative animate-slide-up mt-12">
          ...
          <div
            ref={cardRef}
            className="relative w-full aspect-[2/3] border-[6px] border-black bg-black shadow-2xl touch-pan-y swipe-card"
            style={{
          ...
          >
            <OptimizedPoster
              ...
              priority // First card should load immediately
            />
          ...
          {loading ? (
            ...
            <div>
              <EmptyState 
                emoji="🃏"
                title={t('SWIPE_EMPTY')}
                message="No more cards to swipe. Try changing filters or check back later!"
              />
              <Button
                variant="primary"
                size="md"
