...
export const DiscoverTab = memo(function DiscoverTab({
        ...
        if (filterMode !== 'top') {
          ...
          if (antiRut) {
            ...
            for (const seed of seeds) {
              ...
              // Get recommendations from TMDB - these are actually similar by genre/themes
              ...
              if (mainData.results) {
                // TMDB recommendations are genre-based, so we label them correctly
                allRecs.push(
              ...
              // Also get "similar" movies which are more genre/theme based
              const similarRes = await fetch(
                `${TMDB_BASE_URL}/${endpoint}/${seed.tmdbData.id}/similar?api_key=${apiKey}&language=${language}`,
                { signal }
              );
              const similarData = await similarRes.json();
              if (similarData.results) {
                allRecs.push(
                  ...similarData.results.slice(0, 5).map((r: TMDBData) => ({
                    ...r,
                    seedTitle: seed.title,
                    source: 'similar' as const,
                  }))
  ...
  if (highRatedItems.length < 3 && filterMode === 'top' && !antiRut) {
    return (
      <EmptyState
        icon="search"
        title={t('DISCOVER_LOCKED')}
        message={`Rate ${3 - highRatedItems.length} more items 4+ stars to unlock personalized recommendations`}
      />
    );
  }
        ...
        {visibleRecommendations.map((rec) => (
          ...
          >
            ...
            <div className="p-2 flex flex-col gap-2 flex-1">
              ...
              {rec.seedTitle && rec.source === 'similar' && (
                <p className="text-[8px] text-gray-400 truncate">
                  Similar to {rec.seedTitle}
                </p>
              )}
              ...
              <div className="grid grid-cols-2 gap-1 mt-auto">
                <button
                  onClick={() => onAdd(rec)}
                  className="py-2 bg-white text-black text-[9px] font-bold uppercase hover:bg-gray-100 text-center rounded"
                >
                  + ADD
                </button>
                <button
                  onClick={() => onAddToWatchlist(rec)}
                  className="py-2 border border-white text-white text-[9px] font-bold uppercase hover:bg-white hover:text-black text-center rounded"
                >
                  SAVE
                </button>
              </div>
              <button
                onClick={() => onSeen(rec)}
                className="py-2 bg-gray-700 text-gray-300 text-[9px] font-bold uppercase hover:bg-gray-600 hover:text-white text-center w-full rounded mt-1"
              >
                {t('BTN_SEEN')}
              </button>
            </div>
